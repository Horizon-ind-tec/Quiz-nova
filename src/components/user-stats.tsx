...
const RANK_CONFIG = {
  Bronze: { color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200', next: 1000 },
  Silver: { color: 'text-slate-400', bg: 'bg-slate-50', border: 'border-slate-200', next: 5000 },
  Gold: { color: 'text-amber-500', bg: 'bg-amber-50', border: 'border-amber-200', next: 15000 },
  Platinum: { color: 'text-cyan-500', bg: 'bg-cyan-50', border: 'border-cyan-200', next: 50000 },
  Diamond: { color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200', next: 100000 },
};

export function UserStats({ profile }: UserStatsProps) {
  if (!profile) return null;

  const config = RANK_CONFIG[profile.rank || 'Bronze'];
  const nextRankPoints = config.next;
  const progress = Math.min(100, (profile.points / nextRankPoints) * 100);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <Card className={cn("border-2 shadow-sm", config.bg, config.border)}>
        <CardContent className="p-4 flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Current Rank</p>
